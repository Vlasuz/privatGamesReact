...
import {gameCardsDelete} from "../../../redux/actions";
...
const GameDealing = ({setStepOfGame}) => {
    ...
    const [isGiveLeft, setIsGiveLeft] = useState(true)
    ...
    // GIVE ONE BY ONE CARDS EVERYONE
    const handleChooseCard = (e) => {
        setIsStepMe(true)
        document.querySelectorAll('.game-user-cards__item._active')?.forEach(li => li.classList.remove('_active'))
        e.target.closest('li').classList.add('_active')
        setCardSelect(e.target.closest('li').getAttribute('data-id'))
    }

    const giveSomeone = () => {
        setTime(10)
        if(!isGiveLeft){
            document.querySelector('.game-user-cards__item._active').style.transform = 'translate(100px, -50px) scale(0)'
        } else {
            document.querySelector('.game-user-cards__item._active').style.transform = 'translate(-100px, -50px) scale(0)'
        }
        setTimeout(() => {
            ...
            dispatch(gameCardsDelete(cardSelect))
            document.querySelector('.game-user-cards__item._active').style.transform = 'translate(0px, 0px) scale(1)'
            document.querySelectorAll('.game-user-cards__item._active')?.forEach(li => li.classList.remove('_active'))
            setIsGiveLeft(false)
        }, 300)
    }
                                ...
                                <div className="one-thousand__user game__user">
                                    ...
                                    <div className="game__user--info">
                                        <div className="game__user--info-body">
                                            ...
                                            <progress className="game__user--progress" max="100" value={time * 10}></progress>
                                        </div>
                                    </div>
                                    ...
                                    isStepMe ?
                                        <>
                                            ...
                                            <div className="game__user-menu--row">
                                                <button onClick={giveSomeone} className="game__user-menu--main-btn btn _large _shadow"
                                                        type="button">
                                                    { isGiveLeft ? "Дать игроку слева" : "Дать игроку справа" }
                                                </button>
                                            </div>
                                        </> : ""
                                }
