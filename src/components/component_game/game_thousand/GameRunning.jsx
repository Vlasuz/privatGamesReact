...
import PopupBgd from "../../components_popups/PopupBgd";
import PopupCross from "../../components_popups/PopupCross";
import {useParams} from "react-router-dom";
import {useDispatch, useSelector} from "react-redux";
import Timer from "./Timer";
import {resultOfGame} from "../../../redux/actions";
import {gameResultReducer} from "../../../redux/reducers/gameResultReducer";

const GameRunning = ({setStepOfGame}) => {
    ...
    const [fff, setFff] = useState(true)

    const [numOfCards, setNumOfCards] = useState([])

    const [lftNewResult, setLftNewResult] = useState(0)
    const [rhtNewResult, setRhtNewResult] = useState(0)
    const [myNewResult, setMyNewResult] = useState(0)

    const [isStepLft, setIsStepLft] = useState(false)
    const [isStepRht, setIsStepRht] = useState(false)
    const [isMyStep, setIsMyStep] = useState(true)
    const [step, setStep] = useState(0)
    const [whoIsWinner, setWhoIsWinner] = useState('')

    const cards = useSelector(state => {
        const {cardsListReducer} = state
        return cardsListReducer.cards
    })

    const dispatch = useDispatch()

    const scoreOfGame = useSelector(state => {
        const {gameResultReducer} = state;
        return gameResultReducer.result
    })

    useEffect(() => {
        gameWinner(whoIsWinner)
        if (document.querySelectorAll('.game-user-cards__item').length <= 0 && fff) {
            showResult()
            setFff(false)
        }
    }, [whoIsWinner])
    ...
    const lft_card = [
        {id: 0, card_type: 12, card_suit: 1},
        {id: 1, card_type: 13, card_suit: 1},
        {id: 2, card_type: 9, card_suit: 4},
        {id: 3, card_type: 14, card_suit: 3},
        {id: 4, card_type: 10, card_suit: 1},
        {id: 41, card_type: 9, card_suit: 2},
    ];
    const rht_card = [
        {id: 0, card_type: 10, card_suit: 3},
        {id: 1, card_type: 9, card_suit: 2},
        {id: 2, card_type: 12, card_suit: 2},
        {id: 3, card_type: 13, card_suit: 3},
        {id: 4, card_type: 11, card_suit: 2},
        {id: 42, card_type: 9, card_suit: 3},
    ];

    const sendCard = (e, type, suit) => {
        ...
        if (isMyStep) {
            ...
            let coodItemX = card.offsetLeft
            document.querySelectorAll('.one-thousand__cards--item')[2].style.left = `calc(50% - 170px + ${coodItemX}px)`;
            document.querySelectorAll('.one-thousand__cards--item')[2].style.top = "calc(274px)";
            document.querySelectorAll('.one-thousand__cards--item')[2].style.transform = "translate(-50%,-25%)";
            document.querySelectorAll('.one-thousand__cards--item')[2].style.opacity = 1;
            setTimeout(() => {
                document.querySelectorAll('.one-thousand__cards--item')[2].style.transition = "left .3s ease, top .3s ease";
                document.querySelectorAll('.one-thousand__cards--item')[2].style.left = `50%`;
                document.querySelectorAll('.one-thousand__cards--item')[2].style.top = "0px";
                document.querySelectorAll('.one-thousand__cards--item')[2].style.transform = "translate(-50%,-25%)";
            }, 100)
            card.style.opacity = 0
            setTimeout(() => {
                card.remove()
                document.querySelectorAll('.one-thousand__cards--item')[2].style.transition = "none";
            }, 500)
            document.querySelectorAll('.one-thousand__cards--item')[2].innerHTML = `
                <div  class="one-thousand__cards--item-body">
                    <img src="${image}" alt="" class="one-thousand__cards--img">
                </div>
            `
            ...
            setTimeout(() => {
                ...
                document.querySelectorAll('.one-thousand__cards--item')[0].style.left = `-182px`;
                document.querySelectorAll('.one-thousand__cards--item')[0].style.top = "108px";
                document.querySelectorAll('.one-thousand__cards--item')[0].style.transform = "scale(.5) translate(-125%,-50%)";
                document.querySelectorAll('.one-thousand__cards--item')[0].style.opacity = 1;
                setTimeout(() => {
                    document.querySelectorAll('.one-thousand__cards--item')[0].style.transition = "left .3s ease, top .3s ease, transform .3s ease";
                    document.querySelectorAll('.one-thousand__cards--item')[0].style.left = `50%`;
                    document.querySelectorAll('.one-thousand__cards--item')[0].style.top = "0px";
                    document.querySelectorAll('.one-thousand__cards--item')[0].style.transform = "scale(1) translate(-125%,-50%)";
                }, 100)
                card.style.opacity = 0
                setTimeout(() => {
                    card.remove()
                    document.querySelectorAll('.one-thousand__cards--item')[0].style.transition = "none";
                }, 500)

                document.querySelectorAll('.one-thousand__cards--item')[0].innerHTML = `
                    <div class="one-thousand__cards--item-body">
                        <img src="../images/cards/${lft_card[step].card_type}-${lft_card[step].card_suit}.svg" alt="" class="one-thousand__cards--img">
                    </div>
                `
                ...
                setTimeout(() => {
                    ...
                    document.querySelectorAll('.one-thousand__cards--item')[1].style.left = `518px`;
                    document.querySelectorAll('.one-thousand__cards--item')[1].style.top = "108px";
                    document.querySelectorAll('.one-thousand__cards--item')[1].style.transform = "scale(.5) translate(25%,-50%)";
                    document.querySelectorAll('.one-thousand__cards--item')[1].style.opacity = 1;
                    setTimeout(() => {
                        document.querySelectorAll('.one-thousand__cards--item')[1].style.transition = "left .3s ease, top .3s ease, transform .3s ease";
                        document.querySelectorAll('.one-thousand__cards--item')[1].style.left = `50%`;
                        document.querySelectorAll('.one-thousand__cards--item')[1].style.top = "0px";
                        document.querySelectorAll('.one-thousand__cards--item')[1].style.transform = "scale(1) translate(25%,-50%)";
                    }, 100)
                    card.style.opacity = 0
                    setTimeout(() => {
                        card.remove()
                        document.querySelectorAll('.one-thousand__cards--item')[1].style.transition = "none";
                    }, 500)
                    document.querySelectorAll('.one-thousand__cards--item')[1].innerHTML = `
                        <div class="one-thousand__cards--item-body">
                            <img src="../images/cards/${rht_card[step].card_type}-${rht_card[step].card_suit}.svg" alt="" class="one-thousand__cards--img">
                        </div>
                    `

                    setTimeout(() => {

                        let rht_type = rht_card[step].card_type
                        let rht_suit = rht_card[step].card_suit

                        let lft_type = lft_card[step].card_type
                        let lft_suit = lft_card[step].card_suit
    ...
    function loginOfGame(meCard, lftCard, rhtCard) {

        if (meCard.suit === lftCard.lft_suit || meCard.suit === rhtCard.rht_suit) {

            if (meCard.suit === lftCard.lft_suit) {
                let maxLength = Math.max(length_of_type(meCard.type), length_of_type(lftCard.lft_type))
                setWhoIsWinner(maxLength === length_of_type(meCard.type) ? "me" : "lft")
            }

            if (meCard.suit === rhtCard.rht_suit) {
                let maxLength = Math.max(length_of_type(meCard.type), length_of_type(rhtCard.rht_type))
                setWhoIsWinner(maxLength === length_of_type(meCard.type) ? "me" : "rht")
            }

            if (lftCard.lft_suit === rhtCard.rht_suit) {
                let maxLength = Math.max(length_of_type(meCard.type), length_of_type(rhtCard.rht_type), length_of_type(lftCard.rht_type))

                if (maxLength === length_of_type(meCard.type)) {
                    setWhoIsWinner("me")
                } else {
                    let maxLength = Math.max(length_of_type(lftCard.lft_type), length_of_type(rhtCard.rht_type))
                    setWhoIsWinner(maxLength === length_of_type(lftCard.lft_type) ? "lft" : "rht")
                }
            }
        ...
        setNumOfCards([meCard.type, lftCard.lft_type, rhtCard.rht_type])
    ...
    function gameWinner(winner) {
        ...
        document.querySelectorAll('.one-thousand__cards--item').forEach((item, itemNum) => {
            item.style.transition = "left .3s ease, top .3s ease, transform .3s ease";

            if (winner === 'me') {
                item.style.top = '130px'
                if(oo === 0){
                    oo = 1
                    numOfCards.map(num => setMyNewResult(prev => prev + length_of_type(num)))
                }
            } else if (winner === 'lft') {
                item.style.left = '-250px'
                if(oo === 0){
                    oo = 1
                    numOfCards.map(num => setLftNewResult(prev => prev + length_of_type(num)))
                }
            } else if (winner === 'rht') {
                item.style.left = '500px'
                if(oo === 0){
                    oo = 1
                    numOfCards.map(num => setRhtNewResult(prev => prev + length_of_type(num)))
                }
            }

            if (itemNum === 0) {
                item.style.transform = 'scale(0.4) translateY(10px)';
            } else if (itemNum === 2) {
                item.style.transform = 'scale(0.4) translateY(-10px)';
            } else {
    ...
    function showResult() {
        document.querySelector("#one-thousand-popup").classList.add('_active')

        dispatch(resultOfGame([lftNewResult, rhtNewResult, myNewResult]))
    ...
    return (
        <main className="main">
                ...
                <div className="one-thousand__container game__container container">
                    ...
                    <div className="one-thousand__main">
                        ...
                        <div className="one-thousand__main--grid one-thousand__grid">
                                ...
                                <div className="game__player">
                                    <div className="game__player--body">
                                        ...
                                        <div className="game__player--info">
                                            ...
                                            {isStepLft ? <progress className="game__user--progress" max="100" value={time * 10}></progress> : "" }
                                        </div>
                                ...
                                <div className="game__player">
                                    <div className="game__player--body">
                                        ...
                                        <div className="game__player--info">
                                            ...
                                            {isStepRht ? <progress className="game__user--progress" max="100" value={time * 10}></progress> : "" }
                                        </div>
                                        <div className="game__player--block">
                                            {/*<div className="game__player--min-message">*/}
                                            {/*    Пас*/}
                            ...
                            <div className="one-thousand__grid--item">
                                <div className="one-thousand__user game__user">
                                    <div className="game__user--block">
                                    </div>
                                    <div
                                        className="one-thousand__user--cards game__user--cards game-user-cards _min-space">
                                        <ul className="game-user-cards__list">

                                            {
                                                cards.sort((a, b) => a.card_suit - b.card_suit).map((item, itemNum) =>
                                                    <li onClick={e => sendCard(e, item.card_type, item.card_suit)}
                                                        key={itemNum}
                                    ...
                                    <div className="game__user--info">
                                        <div className="game__user--info-body">
                                            ...
                                            {isMyStep ? <progress className="game__user--progress" max="100" value={time * 10}></progress> : ""}
                                        </div>
                                    </div>
                        ...
                        <div className="one-thousand__main--table one-thousand__table game__table">
                            ...
                            <div className="one-thousand__table--cards one-thousand__cards">
                                <ul className="one-thousand__cards--list">
                                    <li className="one-thousand__cards--item">
                        ...
                        <div className="game__main--user-menu game__user-menu">
                            ...
                            <div className="game__user-menu--col">
                                ...
                                {/*<div className="game__user-menu--row">*/}
                                {/*    <button onClick={() => setBid(prev => prev + 5)} className="game__user-menu--main-btn btn _large _shadow" type="button">*/}
                                {/*        {bid + 5}*/}
                                {/*    </button>*/}
            ...
            <div className="one-thousand-popup popup" id="one-thousand-popup">
                <div className="one-thousand-popup__wrapper popup-wrapper">
                    <PopupBgd/>
                    <div className="one-thousand-popup__body popup-body">
                        <div className="one-thousand-popup__container popup-container">
                            <PopupCross/>
                            <ul className="one-thousand-popup__list">
                                <li className="one-thousand-popup__player">
                                    <div className="one-thousand-popup__player--avatar">
                                        <img src="../images/account/avatar.png" alt=""
                                             className="one-thousand-popup__player--avatar-img"/>
                                    </div>
                                    <div className="one-thousand-popup__player--info">
                                        <div className="one-thousand-popup__player--info-header">
                                            <h3 className="one-thousand-popup__player--name">
                                                Игорь
                                            </h3>
                                            <span>
                                                {lftNewResult}
                                        ...
                                        <span className="one-thousand-popup__player--param">
                                            {scoreOfGame.lft}
                                        </span>
                                        {/*<span className="one-thousand-popup__player--param">160</span>*/}
                                    </div>
                                </li>
                                <li className="one-thousand-popup__player">
                                    <div className="one-thousand-popup__player--avatar">
                            ...
                            <button onClick={() => document.querySelectorAll(".popup").forEach(popup => popup.classList.remove('_active'))} className="hearts-round-popup__btn btn popup-btn popup-close _green">
                                Ок
                            </button>
                        </div>
                    </div>
